...
from observable import ObservableMeter, Energy, Magnetization, Susceptibility, AbsMagnetization, SpecificHeat,\
    get_impsamp_statistics
from quantum_observable import q_ObservableMeter, q_Energy, q_Susceptibility, q_SpecificHeat, plaquette_dist, q_Weight
from utils import show_setups_q
from importsamp import ImportanceSampler
...
def main():
    ...
    argparser.add_argument('--output', type=str, default='results_imp_samp', help='path to store the results')
    ...
    #argparser.add_argument('--on_cluster', default=True)
    ...
    if model_param["sampler"]=="pixelcnn":
        q = PixelCNN(L=model_param["lattice_size"], net_depth=model_param["convs"] if model_param["resnet"] else 3, net_width=model_param["nr_filter"],
                    spin_model=model_param["spin_model"], q=model_param["q"] if model_param["spin_model"]=="potts" else None,
    ...
    if model_param["spin_model"]=="ising":
        meter = ObservableMeter(
            ...
            tag="ImportanceSampling {}".format(str(q)),
        ...
        meter = q_ObservableMeter(
            ...
            args.sample_beta, model_param["j"], model_param["lattice_size"][0]/2 if not q.horizontal_trotter else model_param["lattice_size"][1]/2, q.horizontal_trotter,
            device
        )

    sampler = ImportanceSampler(q.eval(), meter, args.sample_beta, model_param["lattice_size"], args.num_configs, args.lattice, args.ham)
    logging.info(sampler.run(args.n_iter))

    if(args.model_exist):
        sampler.obs_meter.save(os.path.join(args.outputdir, args.output, "sampling_beta{0}_lattice{1}.txt").format(str(args.sample_beta).replace(".",""), model_param["lattice_size"]),
        sample_spec=vars(args))
    else:
        sampler.obs_meter.save(os.path.join(args.outputdir, args.model_dir, args.output, "sampling_beta{0}_lattice{1}.txt").format(str(args.sample_beta).replace(".",""), model_param["lattice_size"]),
        sample_spec=vars(args))
